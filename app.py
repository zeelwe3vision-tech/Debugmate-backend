...
def call_openrouter(messages, model='openai/gpt-4o-mini', temperature=0.5, max_tokens=300):
    """
    Calls the OpenRouter API with safe timeout, retry, and error handling.
    """

    url = "https://openrouter.ai/api/v1/chat/completions"
    mdl = model or os.getenv("OPENROUTER_MODEL", "openai/gpt-4o-mini")

    # ✅ Limit message history to last 6 (prevents oversized payloads)
    if len(messages) > 6:
        messages = messages[-6:]
    ...
    payload = {
        ...
        "max_tokens": int(os.getenv("OPENROUTER_MAX_TOKENS", max_tokens))
    }

    try:
        # ✅ First attempt with 40s timeout
        ...
        print("⏳ OpenRouter timeout — retrying once...")
        try:
            ...
            print("❌ OpenRouter timeout again.")
            return "⚠️ The AI service is taking too long to respond right now. Please try again in a few seconds — everything else is working fine. 🙂"
        except RequestException as e:
            print("⚠️ OpenRouter connection error on retry:", e)
            return f"⚠️ AI connection issue: {str(e)}"

    except RequestException as e:
        print("⚠️ OpenRouter connection error:", e)
        return f"⚠️ AI connection issue: {str(e)}"
