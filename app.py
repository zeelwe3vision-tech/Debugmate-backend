...
# Pinecone client for internal documents
import pinecone

PINECONE_API_KEY = "YOUR_PINECONE_API_KEY"
PINECONE_ENV = "us-east-1"  # e.g., "us-west1-gcp"
PINECONE_INDEX_NAME = "debugmate"

pinecone.init(api_key=PINECONE_API_KEY, environment="us-east-1")

if PINECONE_INDEX_NAME not in pinecone.list_indexes():
    pinecone.create_index(PINECONE_INDEX_NAME, dimension=1024, metric="cosine")  # adjust dimension as needed
...
# ---------------- Document Processing ----------------
# --- Pinecone-based Internal Document RAG ---
...
if PINECONE_INDEX_NAME not in pinecone.list_indexes():
    pinecone.create_index(PINECONE_INDEX_NAME, dimension=1024, metric="cosine")
pinecone_index = pinecone.Index(PINECONE_INDEX_NAME)
...
def load_documents():
    """
    Loads internal documents from 'company_docs', splits them, and upserts embeddings to Pinecone.
    """
    documents = []
    ...
    if documents:
        ...
        for i, text in enumerate(texts):
            ...
            meta = {
                "source": text.metadata.get("source", "company_docs"),
                "text": text.page_content
            }
            pinecone_vectors.append((f"doc_{i}", vector, meta))
        # Upsert in batches
...
def get_context(query, k=3):
    """
    Given a user query, retrieves top-k relevant document chunks from Pinecone using RAG.
    """
    if len(query.split()) <= 2:
        return ""
    try:
        query_vec = embeddings.embed_query(query)
        results = pinecone_index.query(vector=query_vec, top_k=k, include_metadata=True)
        if results and results.matches:
            return "\n".join([match.metadata.get("text", "") for match in results.matches])
    except Exception as e:
        print("Pinecone RAG error:", e)
        return ""
    return ""
